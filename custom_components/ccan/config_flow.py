...
STEP_CCAN_DATA_SCHEMA = vol.Schema(
    {
        vol.Required(CONF_HOST, description={"suggested_value": "192.168.2.14"}): str,
        vol.Required(CONF_PORT, description={"suggested_value": 6020}): int,
    }
)
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    success = await CCAN_Coordinator.validate_connection(
...
class CCANConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        return self.async_show_form(
            step_id="user", data_schema=STEP_CCAN_DATA_SCHEMA, errors=errors
        )
